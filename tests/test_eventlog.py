...
class EventLog(EventCode):
  ...
  @staticmethod
  def attr2col(attr):
    return EventState.State.attr_name_to_col[attr]

  def get_event_data(self, event_code):
  ...
  def _flt_group_by(self, flt_data, grpby_col, sum_col=0):
    assert grpby_col in [self.attr2col(attr) for attr in ['ent_id', 'population_id']], \
          "Invalid group by column"
    assert sum_col in [self.attr2col(attr) for attr in ['id','val_1','val_2','val_3']], \
  ...
  def count_group_by(self, event_code, grpby_attr, **kwargs):
    ...
    data = self.get_event_data(event_code)
    sum_col = self.attr2col('id') # counting is the default
    ...
    if event_code in [EventCode.EAT_FOOD, EventCode.DRINK_WATER]:
      ...
      # log.val_1.update(self.style2int[style])
      flt_idx = self.style2int[kwargs['style']] == data[:,self.attr2col('val_1')]
      ...
      flt_idx = (data[:,self.attr2col('val_1')] == kwargs['item_sig'][0]) \
                & (data[:,self.attr2col('val_2')] >= kwargs['item_sig'][1])

    elif event_code in [EventCode.EARN_GOLD, EventCode.SPEND_GOLD, EventCode.GIVE_GOLD]:
      flt_idx = np.ones(data.shape[0], dtype=np.bool_)
      # log.val_1.update(amount)
      sum_col = self.attr2col('val_1') # sum gold amount

    else:
      assert False, "Invalid event code"

    return self._flt_group_by(data[flt_idx], self.attr2col(grpby_attr), sum_col)
...
class TestEventLog(unittest.TestCase):
  ...
  def test_event_logging(self):
    ...
    self.assertEqual({1: 1}, event_log.count_group_by(EventCode.EAT_FOOD, 'ent_id'))
    self.assertEqual({1: 1}, event_log.count_group_by(EventCode.DRINK_WATER, 'population_id'))
    self.assertEqual({2: 1}, event_log.count_group_by(EventCode.ATTACK, 'ent_id', style=Skill.Melee))

    self.assertEqual({4: 1}, event_log.count_group_by(EventCode.CONSUME, 'ent_id', item_sig=(ration_8.ITEM_TYPE_ID, 5)))
    self.assertEqual({}, event_log.count_group_by(EventCode.CONSUME, 'ent_id', item_sig=(ration_8.ITEM_TYPE_ID, 9)))

    self.assertEqual({}, event_log.count_group_by(EventCode.EQUIP, 'ent_id', item_sig=(ration_8.ITEM_TYPE_ID, 9)))
    self.assertEqual({5: 1}, event_log.count_group_by(EventCode.EQUIP, 'ent_id', item_sig=(hat_7.ITEM_TYPE_ID, 5)))

    self.assertEqual({6: 1}, event_log.count_group_by(EventCode.PRODUCE, 'ent_id', item_sig=(ration_2.ITEM_TYPE_ID, 1)))
    self.assertEqual({6: 1}, event_log.count_group_by(EventCode.SELL, 'ent_id', item_sig=(ration_2.ITEM_TYPE_ID, 1)))
    self.assertEqual({6: 1}, event_log.count_group_by(EventCode.BUY, 'ent_id', item_sig=(hat_4.ITEM_TYPE_ID, 1)))
    self.assertEqual({7: 1}, event_log.count_group_by(EventCode.GIVE, 'ent_id', item_sig=(hat_4.ITEM_TYPE_ID, 1)))

    self.assertEqual({7: 6}, event_log.count_group_by(EventCode.EARN_GOLD, 'population_id'))
    self.assertEqual({0: 7}, event_log.count_group_by(EventCode.SPEND_GOLD, 'population_id'))
    self.assertEqual({1: 8}, event_log.count_group_by(EventCode.GIVE_GOLD, 'population_id'))
